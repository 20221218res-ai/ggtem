...
async function main() {
  ...
  const title = `Seller status action ${unique}`;
  let listingId = null;
  ...
  try {
    ...
    let publicPage = await fetchText(`/listings?mode=sell&query=${encodeURIComponent(title)}`);
    assertIncludes(publicPage, listingId, "active listing public visibility");

    const hideResult = await postJson("/api/market/seller-listings", sellerToken, {
      mode: "STATUS",
      listingId,
      action: "HIDE",
    });
    if (hideResult.json.status !== "HIDDEN") {
      throw new Error(`hide action failed: ${JSON.stringify(hideResult.json)}`);
    }

    publicPage = await fetchText(`/listings?mode=sell&query=${encodeURIComponent(title)}`);
    assertNotIncludes(publicPage, listingId, "hidden listing public visibility");

    const hiddenOwnerPage = await fetchText(
      `/my/listings?q=${encodeURIComponent(unique)}&status=HIDDEN`,
      sellerToken,
    );
    assertIncludes(hiddenOwnerPage, listingId, "hidden listing owner management visibility");

    const resumeResult = await postJson("/api/market/seller-listings", sellerToken, {
      mode: "STATUS",
      listingId,
      action: "RESUME",
    });
    if (resumeResult.json.status !== "ACTIVE") {
      throw new Error(`resume action failed: ${JSON.stringify(resumeResult.json)}`);
    }

    publicPage = await fetchText(`/listings?mode=sell&query=${encodeURIComponent(title)}`);
    assertIncludes(publicPage, listingId, "resumed listing public visibility");

    const pauseResult = await postJson("/api/market/seller-listings", sellerToken, {
      mode: "STATUS",
      listingId,
      action: "PAUSE",
    });
    if (pauseResult.json.status !== "PAUSED") {
      throw new Error(`pause action failed: ${JSON.stringify(pauseResult.json)}`);
    }

    publicPage = await fetchText(`/listings?mode=sell&query=${encodeURIComponent(title)}`);
    assertNotIncludes(publicPage, listingId, "paused listing public visibility");
...
async function createListing(client, input) {
  ...
  await client.query(
    `insert into "ListingInventory" (id, "listingId", "totalQuantity", "minimumQuantity", "availableQuantity", "lockedQuantity", "soldQuantity", version, "updatedAt")
     values ($1, $2, 1000, 100, $3, 0, 0, 0, now())`,
    [id("inventory"), listingId, input.availableQuantity],
  );
  return listingId;
}
